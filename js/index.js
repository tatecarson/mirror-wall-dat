(async function () {
  ...
  // events
  async function onAddFriendsAlbum(e) {
    const input = document.getElementById('friends')
    if (input.value.slice(0, 6) === 'dat://') {
      albums.push(input.value)
      window.localStorage.setItem('albums', JSON.stringify(albums))

      location.reload();
    } else {
      updatePrompt(`
            <div id="close-prompt">Close</div>
            <p>Please enter a dat url</p>
            <p> Try: dat://22f853351d9e0df78b43aa1692f180047a44e91b272f421b99f463164018e5a8</p>
          `)
  ...
  function renderApp() {
    ...
    document.querySelectorAll('.add-friends-album').forEach(el => el.addEventListener('click', onAddFriendsAlbum))
  ...
  async function renderAlbums() {
    ...
    // Iterate through each album and send to cables
  ...
  async function getAlbums(album) {
    ...
    const urlPart = `${info.url}/images/`
    const datURLs = [];
    // get all of the images in the album
    const images = await album.readdir('/images')

    // create the dat urls for each image 
    images.forEach(img => {
      datURLs.push(urlPart + img)
    })

    // cablesImages.push(datURLs);
    // console.log('cablesImages', cablesImages)
    // CABLES.patch.setVarValue('dataIn', cablesImages)
  ...
  async function appendAlbum(album) {
    ...
    if (!images.length) {
      ...
      // TODO: use this to return the images
